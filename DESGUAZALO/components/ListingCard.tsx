...
export function ListingCard({ listing }: { listing: Listing }) {
  ...
  return (
    <article className="group race-cut relative flex h-full min-w-0 flex-col overflow-hidden border border-zinc-300 bg-[#fbfbf8] shadow-[6px_6px_0_#dddcd5] transition duration-300 hover:-translate-y-1 hover:border-zinc-400 hover:shadow-[9px_9px_0_#c9cbc3]">
      <div className="absolute left-0 top-0 z-20 h-16 w-1 bg-[#b6c18a] transition-all duration-300 group-hover:h-full" />
      ...
      <Link href={`/pieza/${listing.slug}`} className="relative block aspect-[4/3] overflow-hidden bg-zinc-200" aria-label={`Ver ${listing.title}`}>
        {image ? (
          <Image
            ...
            className="object-cover saturate-[.82] transition duration-500 group-hover:scale-[1.035] group-hover:saturate-[.92]"
          />
        ) : (
          <div className="race-grid-bg grid h-full place-items-center bg-[#222723] font-racing text-xs font-black uppercase italic tracking-[.12em] text-zinc-500">Sin foto</div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-black/10" />

        <div className="absolute inset-x-3 top-3 flex items-start justify-between gap-2">
          <span className="race-cut-sm max-w-[65%] truncate bg-[#181b19]/92 px-2.5 py-1 font-racing text-[10px] font-black uppercase italic tracking-[.07em] text-zinc-100 backdrop-blur-sm">
      ...
      <div className="flex flex-1 flex-col p-4 pl-5">
        ...
        <Link href={`/pieza/${listing.slug}`} className="block">
          <h3 className="font-racing line-clamp-2 text-[1.18rem] font-black uppercase italic leading-[1.02] tracking-[-0.025em] text-[#181b19] transition group-hover:text-black sm:text-[1.3rem]">
            {listing.title}
          </h3>
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-5">
          <div>
            ...
            <strong className="font-racing text-3xl font-black italic leading-none tracking-[-0.035em] text-[#181b19]">{formatPrice(listing.price)}</strong>
          </div>
          <span className="flex min-w-0 items-center gap-1.5 text-right text-[10px] font-semibold uppercase tracking-[.035em] text-zinc-500">
            <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 shrink-0 text-[#9c6d5d]" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="2.5"/></svg>
