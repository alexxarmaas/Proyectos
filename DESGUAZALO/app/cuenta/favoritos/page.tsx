...
export default function FavoritesPage() {
  const [rows, setRows] = useState<Listing[]>([]); const [loading, setLoading] = useState(true);
  useEffect(() => { const supabase = getBrowserSupabase(); if (!supabase) { setLoading(false); return; } void (async () => { const { data: auth } = await supabase.auth.getUser(); if (!auth.user) { window.location.href = "/login?next=/cuenta/favoritos"; return; } const { data } = await supabase.from("favorites").select("listing:listings(*, listing_images(public_url, position), seller:profiles!listings_seller_id_fkey(id, display_name, location, phone, whatsapp, avatar_url))").eq("user_id", auth.user.id).order("created_at", { ascending: false }); const listings = (data ?? []).map((row) => row.listing).filter(Boolean) as unknown as Listing[]; setRows(listings); setLoading(false); })(); }, []);
