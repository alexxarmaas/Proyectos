...
export default function MyListingsPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!auth.user) { window.location.href = "/login?next=/cuenta/anuncios"; return; }
      const { data } = await client.from("listings").select("*, listing_images(public_url, position)").eq("seller_id", auth.user.id).order("created_at", { ascending: false });
  ...
  async function logout() { if (!supabase) return; await supabase.auth.signOut(); window.location.href = "/"; }
