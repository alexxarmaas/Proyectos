...
export default function PublishPage() {
  ...
  const [form, setForm] = useState({ title:"", brand:"", model:"", generation:"", year:"", engine:"", category:"Motor", condition:"Buen estado", price:"", location:"", latitude:"", longitude:"", description:"", mileage:"", availableParts:"", referenceCode:"", technicalNotes:"", shipping:false, pickup:true });
  ...
  function applyPhotoSuggestions(values:{title?:string;brand?:string;model?:string;generation?:string;year?:string;engine?:string;category?:string;referenceCode?:string}) {
    setForm((prev)=>({
      ...prev,
      title:prev.title||values.title||"",
      brand:prev.brand||values.brand||"",
      model:prev.model||values.model||"",
      generation:prev.generation||values.generation||"",
      year:prev.year||values.year||"",
      engine:prev.engine||values.engine||"",
      category:values.category||prev.category,
      referenceCode:prev.referenceCode||values.referenceCode||""
    }));
  }
  ...
  function useApproximateLocation(){
    ...
    navigator.geolocation.getCurrentPosition(
      ({coords})=>{
        setForm(prev=>({...prev,latitude:String(approximateCoordinate(coords.latitude)),longitude:String(approximateCoordinate(coords.longitude))}));
        setGeoState("ready");
      },
      ()=>setGeoState("error"),
  ...
  async function publish(event: FormEvent) {
    ...
    try {
      ...
      for (let index = 0; index < prepared.length; index++) {
        ...
        const path = user.id + "/" + listingId + "/" + index + "-" + crypto.randomUUID() + "." + ext;
      ...
      const payload = {
        id: listingId, seller_id: user.id, type, title: form.title.trim(), slug, description: form.description.trim() || null,
        ...
        price: form.price ? Number(form.price) : null, location: form.location.trim(), latitude:form.latitude?Number(form.latitude):null, longitude:form.longitude?Number(form.longitude):null, mileage: form.mileage ? Number(form.mileage) : null,
  ...
  return <div className="publish-shell"><div className="shell narrow">
    ...
    <form onSubmit={publish} className="publish-card">
      {step === 1 && <section>
        ...
        {previews.length > 0 && <><div className="preview-grid">{previews.map((src, i) => <div className="photo-card" key={src} draggable onDragStart={() => setDragIndex(i)} onDragOver={(e) => e.preventDefault()} onDrop={() => { if (dragIndex !== null) movePhoto(dragIndex, i); setDragIndex(null); }}>{i === 0 && <span className="photo-main">Portada</span>}<Image src={src} alt={"Vista previa " + (i + 1)} fill unoptimized /></div>)}</div><div className="photo-toolbar"><span>Consejo: incluye etiqueta OEM, conectores y cualquier desperfecto.</span><span>{files.length}/10</span></div><PhotoAssistant key={files.map(file=>file.name+file.size).join("|")} files={files} onApply={applyPhotoSuggestions} onMakeCover={(index)=>movePhoto(index,0)} /></>}
      </section>}
      ...
      {step === 3 && <section className="stack-form">
        ...
        <div className="publish-geo"><div><strong>Mostrar cercanía</strong><span>Guardamos una posición redondeada (~1 km), nunca tu ubicación exacta.</span></div><button type="button" onClick={useApproximateLocation} disabled={geoState==="loading"}>{geoState==="loading"?"Localizando…":geoState==="ready"?"✓ Ubicación aproximada añadida":"⌖ Añadir ubicación aproximada"}</button>{geoState==="error"&&<small>No se pudo obtener la ubicación. El anuncio puede publicarse igualmente.</small>}</div>
