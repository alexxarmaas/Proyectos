...
export default function PublishPage() {
  ...
  return <div className="publish-shell"><div className="shell narrow"><div className="publish-head"><h1>Publicar anuncio</h1><p>Añade fotos y los datos básicos. Puedes completar los datos técnicos si los conoces.</p><div className="steps"><span className={step >= 1 ? "active" : ""}>1 Fotos</span><span className={step >= 2 ? "active" : ""}>2 Datos</span><span className={step >= 3 ? "active" : ""}>3 Precio y zona</span></div></div>
  <form onSubmit={publish} className="publish-card">
    ...
    {step === 3 && <section className="stack-form"><div className="two-cols"><label>{type === "part" ? "Precio" : "Precio orientativo"}<div className="price-input"><input required={type === "part"} inputMode="decimal" value={form.price} onChange={(e) => set("price")(e.target.value)} placeholder="180" /><span>€</span></div></label><label>Localización<input required value={form.location} onChange={(e) => set("location")(e.target.value)} placeholder="Telde, Gran Canaria" /></label></div><label>Descripción <em>opcional</em><textarea value={form.description} onChange={(e) => set("description")(e.target.value)} rows={4} placeholder="Estado real, desperfectos, qué incluye…" maxLength={1500} /></label>{type === "part" && <div className="optional-block"><strong>Datos técnicos opcionales</strong><div className="two-cols"><label>Referencia OEM<input value={form.referenceCode} onChange={(e) => set("referenceCode")(e.target.value)} placeholder="5G1941036" /></label><label>Notas técnicas<input value={form.technicalNotes} onChange={(e) => set("technicalNotes")(e.target.value)} placeholder="Lado derecho, con módulo…" /></label></div></div>}<div className="deal-note">El pago y la entrega se acuerdan directamente entre comprador y vendedor. DESGUÁZALO no procesa pagos.</div></section>}
